# ZenWal: Theming Zen Browser with PyWal automatically

ZenWal is a framework/guide to setting up Zen & PyWal to work together automatically, giving you the right browser colors to match your theme. It uses a script for that reads the current colors generated by pywal and writes them to your userChrome. You can invoke this anywhere. 
...
Once you have the profile name, update it at the top of `scripts/update-zen-colors.sh`:
...
### 4. Call the script from your wal/wallpaper service (eg. waypaper)

Call `update-zen-colors.sh` from whatever script invokes `wal`. For example, if you use waypaper, add this to your post-command script:
...
(P.S) If you don't have a post-command script in your waypaper config, just make a new one with the above code and point to it in your waypaper config. I'd also include `pkill waybar` and `waybar` to restart the waybar, or whatever other status bar service you use, if it relies on pywal.
...
**Why I use `!important`**: Zen's JS theme engine sets some variables via inline styles at runtime, which beats normal `:root` declarations. `!important` forces the override. For some elements (`#zen-main-app-wrapper`, `.zen-browser-generic-background::after`), the variable itself gets overridden, and setting `background` directly on the element works out. This is just speculation, so please correct me if there's a better way or the logic is wrong.
